...
#define MEM_SIZE			2*1024   //内存池的大小为2kb

static char mem[MEM_SIZE];			//定义用来分配内存的数组

#define MEM_START  &mem[0]				//定义内存池的首地址
#define MEM_END	   &mem[MEM_SIZE]		//定义内存池的尾地址


enum  USE_STA{						//定义内存块的使用状态(UNUSED未使用），（USED为使用）
	UNUSED=0,
	USED=1
};
...
typedef struct mem_block{			//定义内存管理块的数据结构
	void 	*mem_ptr;				//当前内存块的地址
	struct mem_block *nxt_ptr;		//下一个内存管理块的地址
	unsigned int mem_size;			//当前内存块大小
	enum USE_STA mem_sta;			//当前内存块使用状态
}mem_block;
#pragma pack()
